...
public value class Optional<out T>(@PublishedApi internal val _value: Any?) {
    ...
    public inline val value: T
        get() = if (_value === EmptyValue) throw IllegalStateException("Missing value") else _value as T

    public inline val isPresent: Boolean
        get() = _value !== EmptyValue
...
public inline fun <T> Optional<T>.filter(predicate: (T) -> Boolean): Optional<T> =
    when {
        _value === Optional.EmptyValue -> Optional.empty()
...
public inline fun <T> Optional<T>.filterNotNull(onEmpty: () -> Optional<T>): Optional<T> =
    when {
        _value === Optional.EmptyValue -> onEmpty()
        _value != null -> this
        else -> onEmpty()
    }
...
public inline fun <T> Optional<T>.filterNotNull(default: Optional<T>): Optional<T> =
    when {
        _value === Optional.EmptyValue -> default
        _value != null -> this
        else -> default
    }
...
public inline fun <T> Optional<T>.filterTo(transform: (T) -> Optional<T>?): Optional<T> =
    when {
        _value === Optional.EmptyValue -> this
        else -> when (val result = transform(_value as T)) {
            null -> Optional.empty()
            else -> result
        }
...
@AvailableSince(KorneaBase.VERSION_1_0_0_ALPHA)
public inline fun <T> Optional<T>.getOrNull(): T? = if (_value === Optional.EmptyValue) null else _value as T

@AvailableSince(KorneaBase.VERSION_1_0_0_ALPHA)
public inline fun <T> Optional<T>.getOrDefault(default: T): T =
    if (_value === Optional.EmptyValue) default else _value as T

@AvailableSince(KorneaBase.VERSION_1_0_0_ALPHA)
public inline fun <T> Optional<T>.getOrElseRun(block: () -> T): T =
    if (_value === Optional.EmptyValue) block() else _value as T

@AvailableSince(KorneaBase.VERSION_1_0_0_ALPHA)
public inline fun <T> Optional<T>.orElse(default: Optional<T>): Optional<T> =
    if (_value === Optional.EmptyValue) default else this