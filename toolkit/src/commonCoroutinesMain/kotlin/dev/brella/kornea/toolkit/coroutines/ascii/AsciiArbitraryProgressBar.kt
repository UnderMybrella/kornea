...
public inline fun CoroutineScope.createArbitraryProgressBar(
    ...
    loadingText: Any? = UNINITIALISED_VALUE,
    loadedText: Any? = UNINITIALISED_VALUE
): Job {
    ...
    return createArbitraryProgressBar(
        ...
        if (loadingText === UNINITIALISED_VALUE) config.defaultLoadingText else loadingText?.toString(),
        if (loadedText === UNINITIALISED_VALUE) config.defaultLoadedText else loadedText?.toString()
    )
}
