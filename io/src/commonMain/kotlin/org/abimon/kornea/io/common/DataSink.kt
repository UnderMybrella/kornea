...
interface DataSink<O: OutputFlow>: ObservableDataCloseable {
    companion object {
        ...
        @ExperimentalUnsignedTypes
        inline fun <reified T> korneaSinkClosed(message: String = "Sink closed"): KorneaResult<T> =
            KorneaResult.errorAsIllegalState(ERRORS_SINK_CLOSED, message)

        @ExperimentalUnsignedTypes
        inline fun <reified T> korneaTooManySinksOpen(capacity: Int): KorneaResult<T> = korneaTooManySinksOpen("Too many flows open (Capacity: $capacity)")
        @ExperimentalUnsignedTypes
        inline fun <reified T> korneaTooManySinksOpen(message: String): KorneaResult<T> =
            KorneaResult.errorAsIllegalState(ERRORS_TOO_MANY_FLOWS_OPEN, message)

        @ExperimentalUnsignedTypes
        inline fun <reified T> korneaSinkUnknown(message: String = "An unknown error has occurred"): KorneaResult<T> =
